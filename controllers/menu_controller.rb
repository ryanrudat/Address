...
class MenuController
  ...
  def search_entries
    ...
    #it will never return an empty string since import_from_csv will fail if an entry does not have name
    match = address_book.binary_search(name)
    system "clear"

    #check if search returned a match. This expression evaluates to false if search returns nil if false.
    #If search finds a amtach then we call a helper method called search_submenu. search_submenu displays
    #a list of operations that can be performed on an Entry. We want to give the user the ability to delete or
    #edit an entry and return to the main menu when a match is found.
    #We write search_submenu
    if match
      puts match.to_s
  ...
  def search_submenu(entry)
    ...
    #use a case statment and take a specific action based on user input.
    #If the user is "d" we call delete_entry and after it returns we call main_menu etc.
    case selection
    when "d"
      system "clear"
      delete_entry(entry)
